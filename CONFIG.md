...
Workspaces and flows embed their title + description into a pgvector
...
The `search` argument of `workspaces` and `flows` then matches a substring of the title **or**
a description whose
...
(`fluss_server/asgi.py` starts it; `embeddings/healer.py` is the loop) in row-locked batches —
...
migration that first nulls the columns (`UPDATE reaktion_workspace SET embedding = NULL,
embedding_model = ''`, likewise `reaktion_flow` — Postgres refuses
